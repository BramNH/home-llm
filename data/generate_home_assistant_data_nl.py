...
with open("piles/pile_of_durations_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    pile_of_durations = { x["duration"]: x["dutch_name"] for x in reader }
    
with open("piles/pile_of_media_names.txt", encoding="utf-8") as f:
    pile_of_media_names = [ x.strip() for x in f.readlines() ]

with open("piles/pile_of_todo_items_dutch.txt", encoding="utf-8") as f:
...
with open("piles/pile_of_device_names_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
...
with open("piles/pile_of_templated_actions_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
...
with open("piles/pile_of_specific_actions_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    pile_of_specific_actions = list(reader)

pile_of_responses = pandas.read_csv("piles/pile_of_responses_dutch.csv")
...
with open("piles/pile_of_status_requests_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    pile_of_status_requests = list(reader)

with open("piles/pile_of_system_prompts_dutch.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
...
def generate_templated_example(template: dict, language: str, persona: str, max_devices: int = 32):
    ...
    question_template: str = template["dutch_phrase"]
...
def generate_example_file(filename: str, seed: int, format_func: Callable, languages: list[str], personas: list[str], *, static_factor: int, template_factor: int, status_request_factor: int):
    ...
    for status_request in tqdm(pile_of_status_requests):
        run_factor_times(generate_status_request, generated_examples, status_request, "nl", "assistant", status_request_factor)
